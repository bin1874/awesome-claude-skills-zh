...
这页不是"文档搬运"，而是给出可直接执行的 Claude Skills 任务路径。建议方式：先选一个明确目标，再套用下面的路径模板执行。
...
去 [技能索引](/skills/) 按关键词筛选，优先选一个"主技能"加一到两个"补位技能"。

### 第 3 步：按平台安装并验证

在 [入门指南](./getting-started) 选择 Claude.ai、Claude Code 或 API 路径，先跑一个最小示例。
