...
**始终先使用 `--help` 运行脚本** 查看用法。在尝试运行脚本并发现确实需要定制解决方案之前，不要阅读源代码。这些脚本可能非常大，会污染上下文窗口。它们的存在是为了作为黑盒脚本直接调用，而不是被读入上下文窗口。
...
创建自动化脚本时，仅包含 Playwright 逻辑（服务器自动管理）：
```python
from playwright.sync_api import sync_playwright

with sync_playwright() as p:
    browser = p.chromium.launch(headless=True) # 始终在无头模式启动 chromium
...
- **将捆绑脚本用作黑盒** - 要完成任务，考虑 `scripts/` 中的脚本是否有帮助。这些脚本可靠地处理常见的复杂工作流，不会污染上下文窗口。使用 `--help` 查看用法，然后直接调用。
